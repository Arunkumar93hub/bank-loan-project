...
select concat(round(sum(loan_amnt) / 10000000,2), 'Cr') as total_loan_amount from finance_1;

-- Total Fund Amount
select sum(funded_amnt) from finance_1;
select concat(round(sum(funded_amnt) / 10000000,2), 'Cr') as total_fund_amount from finance_1;

-- Total Revol Balance

select sum(revol_bal) from finance_2;
select concat(round(sum(revol_bal) / 10000000,2), 'Cr') as total_revol_amount from finance_2;
